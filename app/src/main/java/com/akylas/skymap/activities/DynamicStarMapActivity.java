...
import com.akylas.skymap.activities.util.ActivityLightLevelChanger.NightModeable;
import com.akylas.skymap.activities.util.ActivityLightLevelManager;
import com.akylas.skymap.activities.util.FullscreenControlsManager;
import com.akylas.skymap.base.Lists;
import com.akylas.skymap.control.AstronomerModel;
import com.akylas.skymap.control.AstronomerModel.Pointing;
import com.akylas.skymap.control.ControllerGroup;
import com.akylas.skymap.control.LocationController;
import com.akylas.skymap.control.MagneticDeclinationCalculatorSwitcher;
import com.akylas.skymap.inject.HasComponent;
import com.akylas.skymap.layers.LayerManager;
import com.akylas.skymap.math.CoordinateManipulationsKt;
...
import com.akylas.skymap.touch.DragRotateZoomGestureDetector;
import com.akylas.skymap.touch.GestureInterpreter;
import com.akylas.skymap.touch.MapMover;
import com.akylas.skymap.util.MiscUtil;
import com.akylas.skymap.util.SensorAccuracyMonitor;
import com.akylas.skymap.views.ButtonLayerView;
...
//  @Inject LocationController locationController;
  ...
  private boolean einkMode = false;
    ...
    ActivityLightLevelChanger activityLightLevelChanger = new ActivityLightLevelChanger(this,
        new NightModeable() {
          ...
          public void setEInkMode(boolean eink) {
            DynamicStarMapActivity.this.rendererController.queueEInkVisionMode(eink);
          }
    });
    activityLightLevelManager = new ActivityLightLevelManager(activityLightLevelChanger,
                                                              sharedPreferences);
    einkMode = activityLightLevelManager.einkMode;
    nightMode = activityLightLevelManager.currentMode == ActivityLightLevelManager.LightMode.NIGHT;
...
//      Log.i(TAG, "Minimum sensors present");
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    MenuItem item = menu.findItem(R.id.menu_item_dim);
    item.setChecked(nightMode);
    item = menu.findItem(R.id.menu_item_eink);
    item.setChecked(einkMode);
    return true;
  }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    fullscreenControlsManager.delayHideTheControls();
      ...
      if (itemId == R.id.menu_item_search) {
        ...
        if (controller.locationController.isRequestingLocation()) {
          controller.locationController.cancelRequestLocation();
        } else {
          controller.locationController.requestLocation();
        }
      ...
      } else if (itemId == R.id.menu_item_eink) {
          Log.d(TAG, "Toggling EInk");
          einkMode = !einkMode;
          sharedPreferences.edit().putBoolean(ActivityLightLevelManager.EINK_KEY, einkMode).commit();
  ...
  public void onStart() {
    ...
    boolean noAutoLocate = sharedPreferences.getBoolean(ApplicationConstants.NO_AUTO_LOCATE, false);
    if (!noAutoLocate) {
  ...
  public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
    ...
    switch (key) {
      ...
      case ApplicationConstants.VIEW_MODE_PREFKEY:
        updateViewDirectionMode(model, sharedPreferences);
      case ApplicationConstants.NO_AUTO_LOCATE:
        boolean noAutoLocate = sharedPreferences.getBoolean(key, false);
        if (!noAutoLocate) {
          requestLocationPermission();
        }
      default:
...
//    Log.i(TAG, "Set up controllers @ " + System.currentTimeMillis());
  ...
  protected void onRestoreInstanceState(Bundle icicle) {
    ...
    einkMode = icicle.getBoolean(ApplicationConstants.BUNDLE_EINK_MODE, false);
  }
  ...
  protected void onSaveInstanceState(Bundle icicle) {
    ...
    icicle.putBoolean(ApplicationConstants.BUNDLE_EINK_MODE, einkMode);
