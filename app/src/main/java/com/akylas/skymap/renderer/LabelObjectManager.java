...
import static com.akylas.skymap.math.MathUtilsKt.DEGREES_TO_RADIANS;
...
import com.akylas.skymap.renderer.util.GLBuffer;
import com.akylas.skymap.renderer.util.LabelMaker;
...
import com.akylas.skymap.util.FixedPoint;
...
public class LabelObjectManager extends RendererObjectManager {
  ...
  public void updateObjects(List<TextPrimitive> labels, EnumSet<UpdateType> updateType) {
    if (updateType.contains(UpdateType.Reset)) {
      ...
      for (int i = 0; i < safeLabels.size(); i++) {
        mLabels[i] = new Label(safeLabels.get(i), getRenderState());
      }
  ...
  private static class Label extends LabelMaker.LabelData {
    public Label(TextPrimitive ts, RenderStateInterface renderState) {
      super(ts.getText(),  renderState.getEInkVisionMode() ? 0xff000000 : 0xffffffff, ts.getFontSize());
  ...
  private void drawLabel(GL10 gl, Label label) {
    ...
    if (getRenderState().getNightVisionMode() || getRenderState().getEInkVisionMode()) {
