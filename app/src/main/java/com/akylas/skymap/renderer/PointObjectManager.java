...
import com.akylas.skymap.renderer.util.NightVisionColorBuffer;
import com.akylas.skymap.renderer.util.SkyRegionMap;
import com.akylas.skymap.renderer.util.TexCoordBuffer;
...
public class PointObjectManager extends RendererObjectManager {
  ...
  public void updateObjects(List<PointPrimitive> points, EnumSet<UpdateType> updateType) {
    ...
    for (RegionData data : mSkyRegions.getDataForAllRegions()) {
      ...
      for (PointPrimitive p : data.sources) {
        int color = (getRenderState().getEInkVisionMode() ? 0xffffffff :  0xff000000) | p.getColor();  // Force alpha to 0xff
  ...
  protected void drawInternal(GL10 gl) {
    ...
    for (RegionData data : activeRegionData) {
      ...
      data.mColorBuffer.set(gl, getRenderState().getNightVisionMode() || getRenderState().getEInkVisionMode());
